...
import { Station as S } from "@/lib/contracts/station/station";
...
const ModalData: React.FC<{ targetID: string }> = ({ targetID }) => {
  const { updateStation, marker, updateError } = useContext(
    StationContext
  ) as AppContext;
  ...
  const [stationName, setStationName] = useState<string>(
    `Station ${targetID} data is not available`
  );
  const stationID = targetID.toString();
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedData: { stations: Station[] }[] = await fetchStation(
          stationID
        );
  ...
  useEffect(() => {
    ...
    for (const date of dateList) {
      ...
      const sensorList = selectedData.map(
        (sensor: any): SensorOption => ({
...
const Modal: React.FC<ModalProps> = ({
  ...
  const { station } = useContext(StationContext) as AppContext;
  ...
  const filteredDates = [
    ...new Set(dateList.map((date) => date.split("T")[0])),
  ];
    ...
    if (selectedSensors.length > 0) {
      ...
      const sensorsWithinTimeFrame = (timeFrame: string): StationData[] => {
        return sensors.filter((sensor) => {
          const sensorToDate = new Date(sensor.date)
            .toISOString()
            .split("T")[0];
          const sensorYesterday = new Date(
            new Date().setDate(new Date().getDate() - 1)
          )
            .toISOString()
            .split("T")[0];
          const sensorOneWeekAgo = new Date(
            new Date().setDate(new Date().getDate() - 7)
          )
            .toISOString()
            .split("T")[0];
          const sensorOneMonthAgo = new Date(
          ...
          if (timeFrame === "today") {
            ...
            return (
              sensorToDate === desiredDate || sensorToDate === sensorYesterday
            );
          } else if (timeFrame === "last-week") {
            return (
              sensorToDate >= sensorOneWeekAgo && sensorToDate <= todayDate
            );
          } else if (timeFrame === "last-month") {
            return (
              sensorToDate >= sensorOneMonthAgo && sensorToDate <= todayDate
            );
          }
          return sensor;
        });
      ...
      for (const sensor of finalSensors()) {
        const selectedSensorData: SensorOption[] = sensor.data.filter((data) =>
        ...
        generatedData.push({
          date: new Date(sensor.date).toLocaleString(
    ...
    const handleLabelClick = (label: string) => {
      setSelectedSensors((prevSelectedSensors) => {
        if (prevSelectedSensors.includes(label)) {
          ...
          return prevSelectedSensors.filter(
      ...
      sensorList.push(
        <div className="sensor-groups" key={groupName}>
          <h3>
            ...
            {station
              ? station[
                  `direction${parseInt(groupName) + 1}Municipality` as keyof S
                ]?.toString()
              : null}
          </h3>
          {sensorGroups[groupName].map(
            (sensor: SensorOption, index: number) => (
              <div key={index}>
                <input
                  type="checkbox"
                  id={sensor.label}
                ...
                <label
                  htmlFor={sensor.id}
                  onClick={() => handleLabelClick(sensor.label)}
                >
    ...
    <dialog className="modal-data-container">
      <div className="modal-content">
        ...
        <h3 className="station-name">
          {station
            ? station.names[i18n.language as keyof S["names"]]?.toString()
            : null}
        </h3>
