...
export default function DirectionPopup({
  ...
  const { updateStation, stationError } = useContext(
    StationContext
  ) as AppContext;
      ...
      {direction ? (
        ...
        >
          ...
          <ul className={styles.list}>
            <button
              type="button"
              className={stationError ? styles.warning : undefined}
              onClick={() => {
            ...
            {station.sensors?.map((sensor) => {
              ...
              return (
                <li className={styles.li} key={sensorName}>
                  {sensor.value || "-"} {t(unit, { ns: "units" })}{" "}
                  {t(sensorName, { ns: "sensors" })}
                </li>
              );
            })}
          </ul>
